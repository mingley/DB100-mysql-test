...
SELECT primary_key, amount, payment_date FROM payment WHERE payment_date = 05/27/2005;
...
SELECT phone, district FROM address IN (California, England, Taipei, West Java);
...
SELECT * FROM payment WHERE payment_date BETWEEN 05/25/2005 00:00:00 AND 05/26/2005 23:59:59;
...
SELECT title, description, release_year, rental_duration * rental_rate FROM film WHERE description BETWEEN 100 AND 120;
# ---------------------------------------------------------#
...
SELECT title, description, release_year FROM film WHERE description = "A thoughtful%";
...
SELECT title, description, rental duration FROM film WHERE description = '%Boat';
...
SELECT title, length, description, rental rate FROM film WHERE description = '%database%' AND length > 180;
...
SELECT payment_id, payment_date, amount FROM payment_table WHERE amount > 5 AND 51-100;

# 7c. Select all columns from the customer table, limiting results to those where the zero-based index is between 101-200.
SELECT * FROM customer WHERE amount BETWEEN 101 AND 200;
...
SELECT payment_date, amount FROM payment ORDER BY amount DESC;
...
SELECT title, description,special_features, length, rental_duration FROM film LIMIT 10 WHERE length > 10 AND rental_duration = 5-7 ORDER BY DESC;
...
SELECT customer.first_name, customer.last_name, actor.first_name, actor.last_name FROM customer RIGHT JOIN actor ON customer.last_name = actor.last_name
...
SELECT customer.first_name, customer.last_name, actor.first_name, actor.last_name FROM customer INNER JOIN actor ON customer.last_name = actor.last_name
...
SELECT city_id, country_id FROM city LEFT JOIN country ON city.country_id = country.country_id
...
SELECT title, description, release_year, language_id FROM film LEFT JOIN language on language.language_id = film.language_id
...
SELECT first_name, last_name, address, address2, city_id, district, postal_code FROM staff LEFT JOIN address ON address.address = address LEFT JOIN city ON city.city_id = city_id
